...
def generate_statement(filename, data):
    ...
    # ---- Header ----
    header_style = ParagraphStyle('header', fontSize=9, alignment=TA_RIGHT, leading=13)
    bank_style   = ParagraphStyle('bank',   fontSize=14, fontName='Helvetica-Bold', alignment=TA_LEFT)
    ...
    header_info = (
        "Capital L.L. 3,000,000,000,000<br/>"
        "R.C.B: 24783 - List of Banks: 14<br/>"
        "Head Office: Beirut, Lebanon<br/>"
        f"Date of issue: {data['issue_date']}"
    )

    top_table = Table(
        [[Paragraph("BLOM BANK SAL", bank_style),
          Paragraph(header_info, header_style)]],
    ...
    info_data = [
        [Paragraph("<b>Name:</b>", label_style),
         Paragraph(data['name'], value_style),
         Paragraph("<b>Account Number:</b>", label_style),
         Paragraph(data['account_number'], value_style)],
        [Paragraph("<b>Period:</b>", label_style),
         Paragraph(data['period'], value_style),
         Paragraph("<b>Currency:</b>", label_style),
         Paragraph(data['currency'], value_style)],
        [Paragraph("<b>Branch:</b>", label_style),
         Paragraph(data['branch'], value_style),
         Paragraph("<b>IBAN:</b>", label_style),
         Paragraph(data['iban'], value_style)],
    ]
    info_table = Table(info_data, colWidths=[25*mm, 65*mm, 35*mm, 55*mm])
    ...
    header_row = [
        ...
        Paragraph("<b>Document</b>", label_style),
        Paragraph("<b>Transaction</b>", label_style),
        Paragraph("<b>Balance</b>", label_style),
    ]
    col_w = [25*mm, 70*mm, 28*mm, 28*mm, 29*mm]

    tx_rows = [header_row]
    for tx in data['transactions']:
        tx_rows.append([
            Paragraph(tx[0], value_style),
            Paragraph(tx[1], value_style),
            Paragraph(tx[2], value_style),
            Paragraph(tx[3], value_style),
            Paragraph(tx[4], value_style),
        ])
    ...
    elements.append(Paragraph("<i>Except Errors or Omissions</i>", ParagraphStyle('fi', fontSize=8)))
    elements.append(Spacer(1, 3*mm))
    elements.append(Paragraph(
        "This statement will be considered confirmed unless you notify the bank within fifteen days from delivery.<br/>"
...
generate_statement("Finland_Statement.pdf",   finland)
generate_statement("Venezuela_Statement.pdf", venezuela)
generate_statement("Vietnam_Statement.pdf",   vietnam)
