...
<style>
  ...
  .bank-name {{ font-size: 15pt; font-weight: bold; }}
  .bank-info {{ text-align: right; font-size: 8pt; line-height: 1.5; float: right; }}
  h1 {{ text-align: center; font-size: 17pt; letter-spacing: 1px; margin: 10px 0 12px 0; }}
  ...
  table.tx th {{ background: #ddd; border: 0.5px solid #999; padding: 5px 4px;
                 font-weight: bold; text-align: left; }}
  table.tx td {{ border: 0.5px solid #bbb; padding: 4px 4px; }}
  table.tx tr:nth-child(even) td {{ background: #f7f7f7; }}
  .r {{ text-align: right; }}
  .footer {{ margin-top: 18px; font-size: 7.5pt; color: #555; text-align: center; }}
  .footer-italic {{ font-style: italic; font-size: 8pt; color: #333; margin-top: 14px; }}
</style>
</head>
<body>
<div class="top">
  <span class="bank-name">BLOM BANK SAL</span>
...
<table class="info-table">
  <tr>
    <td class="lbl">Name:</td><td>{name}</td>
    <td class="lbl">Account Number:</td><td>{account_number}</td>
  </tr>
  <tr>
    <td class="lbl">Period:</td><td>{period}</td>
    <td class="lbl">Currency:</td><td>{currency}</td>
  </tr>
  <tr>
    <td class="lbl">Branch:</td><td>{branch}</td>
    <td class="lbl">IBAN:</td><td>{iban}</td>
  </tr>
</table>
<table class="tx">
  <tr>
    ...
    <th style="width:14%" class="r">Transaction</th>
    <th style="width:14%" class="r">Balance</th>
  </tr>
  {rows}
</table>
<p class="footer-italic">Except Errors or Omissions</p>
<p class="footer">
...
def make_rows(txs):
    html = ""
    for t in txs:
        html += f"<tr><td>{t[0]}</td><td>{t[1]}</td><td>{t[2]}</td><td class='r'>{t[3]}</td><td class='r'>{t[4]}</td></tr>\n"
    return html

def generate(filename, data):
    rows = make_rows(data["transactions"])
    html = TEMPLATE.format(rows=rows, **{k: v for k, v in data.items() if k != "transactions"})
